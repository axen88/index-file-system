...
int32_t print_one_cache_info(net_para_t *net, ofs_block_cache_t *cache)
{
    ...
    NET_PRINT(net, "cache: %p, vbn: %lld, state: 0x%x, ib: %p\n",
...
int32_t print_one_fs_info(net_para_t *net, container_handle_t *ct)
{
    ...
    NET_PRINT(net, "name: %s, flags: 0x%x, ref: %u\n", ct->name, ct->flags, ct->ref_cnt);
...
int32_t print_one_obj_info(net_para_t *net, object_info_t *obj_info)
{
    ...
    NET_PRINT(net, "objid: %lld, inode_no: %lld, obj_state: 0x%x, ref_cnt: %u, name: %s\n",
...
int32_t print_super_block(char *ct_name, net_para_t *net)
{
    ...
    if (ret < 0)
    {
        NET_PRINT(net, "Open ct failed. ct_name(%s) ret(%d)\n",
            ct_name, ret);
        return ret;
    }
    ...
    NET_PRINT(net, "blk_id                : 0x%X\n",   ct->sb.head.blk_id);
    NET_PRINT(net, "alloc_size            : %d\n",     ct->sb.head.alloc_size);
    NET_PRINT(net, "real_size             : %d\n",     ct->sb.head.real_size);
    NET_PRINT(net, "seq_no                : 0x%04X\n", ct->sb.head.seq_no);
    NET_PRINT(net, "fixup                 : 0x%04X\n\n", ct->sb.head.fixup);

    
    NET_PRINT(net, "block_size_shift      : %d\n", ct->sb.block_size_shift);
    NET_PRINT(net, "block_size            : %d\n", ct->sb.block_size);
    NET_PRINT(net, "sectors_per_block     : %d\n\n", ct->sb.sectors_per_block);
    
    NET_PRINT(net, "total_blocks          : %lld\n\n", ct->sb.total_blocks);
    
    NET_PRINT(net, "objid_id              : %lld\n",  ct->sb.objid_id);
    NET_PRINT(net, "objid_inode_no        : %lld\n\n",  ct->sb.objid_inode_no);
    
    NET_PRINT(net, "space_id              : %lld\n",  ct->sb.space_id);
    NET_PRINT(net, "space_inode_no        : %lld\n",  ct->sb.space_inode_no);
    NET_PRINT(net, "free_blocks           : %lld\n",  ct->sb.free_blocks);
    NET_PRINT(net, "first_free_block      : %lld\n\n",  ct->sb.first_free_block);
    
    NET_PRINT(net, "base_id               : %lld\n",  ct->sb.base_id);
    NET_PRINT(net, "base_inode_no         : %lld\n",  ct->sb.base_inode_no);
    NET_PRINT(net, "base_free_blocks      : %lld\n",  ct->sb.base_free_blocks);
    NET_PRINT(net, "base_first_free_block : %lld\n\n",  ct->sb.base_first_free_block);

    NET_PRINT(net, "base_blk              : %lld\n\n",  ct->sb.base_blk);

    NET_PRINT(net, "snapshot_no           : %lld\n\n",  ct->sb.snapshot_no);
    
    NET_PRINT(net, "flags                 : 0x%08X\n", ct->sb.flags);
    NET_PRINT(net, "version               : %d\n",     ct->sb.version);
    NET_PRINT(net, "magic_num             : 0x%04X\n", ct->sb.magic_num);
...
{
    NET_PRINT(net, "FS info:\n");
    NET_PRINT(net, "-----------------------------------------\n");
    NET_PRINT(net, "name        : %s\n", ct->name);
    NET_PRINT(net, "flags       : 0x%x\n", ct->flags);
    NET_PRINT(net, "ref_cnt     : %u\n", ct->ref_cnt);
...
{
    NET_PRINT(net, "Object info:\n");
    NET_PRINT(net, "-----------------------------------------\n");
    NET_PRINT(net, "objid                  : %lld\n", obj_info->objid);
    NET_PRINT(net, "inode_no               : %lld\n", obj_info->inode_no);
    NET_PRINT(net, "name               : %s\n",   obj_info->name);
    NET_PRINT(net, "state                  : 0x%x\n", obj_info->obj_state);
    NET_PRINT(net, "ref_cnt                : %d\n",   obj_info->ref_cnt);
    NET_PRINT(net, "vbn                    : %lld\n", obj_info->root_cache.vbn);
...
int32_t cmd_list(char *ct_name, uint64_t objid, net_para_t *net)
{
    ...
    if (strlen(ct_name) == 0)
    {
        ...
        if (ret < 0)
        {
            NET_PRINT(net, "Walk all opened ct failed. ret(%d)\n", ret);
        }
    ...
    if (ct == NULL)
    {
        NET_PRINT(net, "The ct is not opened. ct(%s)\n", ct_name);
        return -2;
    }
    ...
    if (obj_info == NULL)
    {
        NET_PRINT(net, "The object is not opened. ct(%p) objid(%lld)\n", ct, objid);
        return ret;
    }
